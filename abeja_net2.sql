-- ======================================================
-- BASE DE DATOS: AbejaNet v2 (con Apiarios)
-- ======================================================
...
-- NUEVA: Tabla para los apiarios (agrupaciones de colmenas)
...
-- MODIFICADA: Tabla para las colmenas/nidos
CREATE TABLE colmenas (
  id                        INT UNSIGNED PRIMARY KEY AUTO_INCREMENT,
  apiario_id                INT UNSIGNED NOT NULL,
  nombre                    VARCHAR(100) NOT NULL UNIQUE, -- Nombre globalmente único para la colmena
  descripcion_especifica    TEXT NULL, -- Descripción específica de la ubicación DENTRO del apiario (ej: "Fila 3, al lado del roble")
  fecha_creacion            TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
  FOREIGN KEY (apiario_id) REFERENCES apiarios(id) ON DELETE CASCADE -- Si se borra un apiario, se borran sus colmenas
);
-- NOTA: Se eliminó la unicidad compuesta (apiario_id, nombre) y se hizo 'nombre' globalmente UNIQUE.

-- NUEVA: Tabla de unión para asignar usuarios a apiarios (Muchos a Muchos)
...
-- ------------------------------------------------------
-- TABLAS DE SENSORES Y LECTURAS (Sin cambios estructurales)
-- ------------------------------------------------------

CREATE TABLE sensores (
  id                  INT UNSIGNED PRIMARY KEY AUTO_INCREMENT,
  mac_address         VARCHAR(17) UNIQUE NULL, -- Dirección MAC única del dispositivo físico (ESP32)
...
-- NOTA: Se añadió mac_address para identificar el hardware. Es UNIQUE porque no puede haber dos sensores con la misma MAC.
...
CREATE TABLE lecturas_ambientales (
  ...
  sonido            DECIMAL(5, 2) NULL, -- Nivel de sonido (ej. en dB)
  lluvia            BOOLEAN NULL,
...
-- Usuarios (sin cambios, como pediste)
...
-- Colmenas de ejemplo (ahora asignadas a un apiario)
-- Asegurarse que los nombres de colmena sean globalmente únicos si se insertan más.
...
-- Asignación de usuarios a apiarios de ejemplo
...
-- Sensores de ejemplo (ahora con MAC address de ejemplo)
INSERT INTO sensores (mac_address, colmena_id, tipo_sensor, estado, fecha_instalacion) VALUES
  ...
  (NULL, NULL, 'General', 'no_asignado', NULL); -- Un sensor registrado pero sin dispositivo físico asignado

-- Lecturas de ejemplo (ahora con sonido)
INSERT INTO lecturas_ambientales (sensor_id, humedad, temperatura, peso, sonido, lluvia) VALUES
  ...
  ((SELECT id FROM sensores WHERE mac_address = 'AA:BB:CC:11:22:44'), NULL, NULL, 15.5, NULL, false), -- Sensor de peso no mide otros valores
